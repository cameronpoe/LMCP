...
with uproot.open(root_file) as f:
    ...
    labels, counts = np.unique(ak.to_numpy(ak.flatten(pore_branches['PDGID'])), return_counts=True)
    print(labels)
    particle_dict = {
        '-12': 'antie- v',
        '-11': 'e+',
    ...
    proc_labels = np.unique(np.array(ak.flatten(pore_branches['CreatorProc', pore_branches['PDGID'] == 11])))
    proc_dict = {
        'compt': 'Comptons',
        'eIoni': 'Ionized by e-',
    ...
    for i, proc_label in enumerate(proc_labels):
        event_cut_by_process = ak.any(pore_branches['CreatorProc'] == proc_label, axis=1)
        proc_counts.append(len(pore_branches['CreatorProc', pore_branches['PDGID'] == 11][event_cut_by_process]))
    ...
    fig, ax = plt.subplots()
    bins = np.linspace(0, 5, 101) # keV - note, this bounds doesn't capture all electrons due to the high energy e- formed scattering capture gammas
    ...
    ax.legend()
    ax.set_xlabel('Initial e- energy (keV)')
    ax.set_ylabel('Percent of total electrons per 50 eV bin')
    ax.xaxis.set_ticks_position('both')
    ax.yaxis.set_ticks_position('both')
    plt.minorticks_on()
    plt.show()
