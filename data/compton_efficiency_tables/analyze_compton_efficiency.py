...
for file_num, file_name in enumerate(os.listdir(data_directory)):
    with uproot.open(data_directory + "/" + file_name) as f:
        ...
        hist_dict[
            f"{energy} keV, $\\theta={theta}\degree$, $\phi={phi}\degree$"
        ] = np.array([bin_centers, npore / nlam * 100])
        ...
        ax.plot(
            domain, bspline(domain), color="red", linewidth=3, label="Cubic spline fit"
        )
        domain = np.linspace(x[-2], 511, 100)
        ax.plot(
            domain,
            np.interp(domain, x[-2:], y[-2:]),
            linewidth=3,
            color="green",
        ...
        round_effs = np.round(effs, decimals=4) + 0.0  # add zero to get rid of -0.0
        eff_string = ""
        ...
        with open(
            os.path.dirname(os.path.realpath(__file__)) + "/" + output_name + ".csv",
            "w",
        ) as f2:
            f2.write(eff_string)
