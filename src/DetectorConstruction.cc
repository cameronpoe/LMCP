...
DetectorConstruction::DetectorConstruction()
    : G4VUserDetectorConstruction(), fEMfield_1(0), fEquation_1(0),
      fStepper_1(0), fFieldMgr_1(0), fMinStep_1(0), fChordFinder_1(0),
      fEMfield_2(0), fEquation_2(0), fStepper_2(0), fFieldMgr_2(0),
      fMinStep_2(0), fChordFinder_2(0), fEMfield_WORLD(0), fEquation_WORLD(0),
      fStepper_WORLD(0), fFieldMgr_WORLD(0), fMinStep_WORLD(0),
...
// Destructor
//****************************************************************************
DetectorConstruction::~DetectorConstruction() { delete fDetMessenger; }
...
void DetectorConstruction::DefineMaterials() {
  ...
  // Schott B33 glass
  G4Material *eSiO2 =
      nistManager->FindOrBuildMaterial("G4_SILICON_DIOXIDE", isotopes);
  G4Material *eB2O3 =
      nistManager->FindOrBuildMaterial("G4_BORON_OXIDE", isotopes);
  G4Material *eNa2O =
      nistManager->FindOrBuildMaterial("G4_SODIUM_MONOXIDE", isotopes);
  G4Material *eAl2O3 =
      nistManager->FindOrBuildMaterial("G4_ALUMINUM_OXIDE", isotopes);
  ...
  // Schott D263M
  G4Material *eZnO = new G4Material("G4_ZINC_OXIDE", 5.61 * g / cm3, 2);
  eZnO->AddElement(eO, 1);
  eZnO->AddElement(eZn, 1);
  G4Material *eK2O =
      nistManager->FindOrBuildMaterial("G4_POTASSIUM_OXIDE", isotopes);
  G4Material *eTiO2 =
      nistManager->FindOrBuildMaterial("G4_TITANIUM_DIOXIDE", isotopes);
  auto mD263M = new G4Material("D263M", 2.51 * g / cm3, 7);
  mD263M->AddMaterial(eSiO2,
                      (64.1 + 0.1) * perCent); // the antimony trioxide doesn't
  mD263M->AddMaterial(eB2O3, 8.4 * perCent);
  ...
  // Ecomass2
  // PA12 with W nano particles (actually a mix if PA12 and PBA, but for
  // simplicity only doing PA12) 81.72% W by weight
...
G4VPhysicalVolume *DetectorConstruction::DefineVolumes() {
  ...
  // auto mLMCP = G4Material::GetMaterial("G4_GLASS_LEAD");
  auto mLMCP = G4Material::GetMaterial("G4_KAPTON");
  ...
  //------------------------------------------------------
  // SOLID VOLUME
  auto world_S = new G4Box("World_S",    // its name
                           world_X / 2,  // its X dimension
                           world_Y / 2,  // its Y dimension
                           world_Z / 2); // its Z dimension

  // LOGICAL VOLUME
  auto world_LV = new G4LogicalVolume(world_S,     // its solid
  ...
  //----------------------------------
  // SOLID VOLUME
  auto LMCP_S = new G4Box("LMCP_S",    // its name
                          lmcp_X / 2,  // its X dimension
                          lmcp_Y / 2,  // its Y dimension
                          lmcp_Z / 2); // its Z dimension

  // LOGICAL VOLUME
  auto LMCP_LV = new G4LogicalVolume(LMCP_S,     // its solid
                                     mLMCP,      // its material
                                     "LMCP_LV"); // its name
  ...
  //----------------------------------
  // SOLID VOLUME
  auto lamina_S = new G4Box("Lamina_S",    // its name
                            lamina_X / 2,  // its X dimension
                            lamina_Y / 2,  // its Y dimension
                            lamina_Z / 2); // its Z dimension

  // LOGICAL VOLUME
  auto lamina_LV = new G4LogicalVolume(lamina_S,     // its solid
                                       mLMCP,        // its material
                                       "Lamina_LV"); // its name

  new G4PVReplica("Lamina_PV", // its name
  ...
  //----------------------------------
  // SOLID VOLUME
  auto divLamina_S = new G4Box("Div_Lamina_S",    // its name
                               div_lamina_X / 2,  // its X dimension
                               div_lamina_Y / 2,  // its Y dimension
                               div_lamina_Z / 2); // its Z dimension

  // LOGICAL VOLUME
  auto divLamina_LV = new G4LogicalVolume(divLamina_S,      // its solid
                                          mLMCP,            // its material
                                          "Div_Lamina_LV"); // its name

  new G4PVReplica("Div_Lamina_PV", // its name
  ...
  //----------------------------------
  // SOLID VOLUME
  auto pore_S = new G4Box("Pore_S",    // its name
                          pore_X / 2,  // its X dimension
                          pore_Y / 2,  // its Y dimension
                          pore_Z / 2); // its Z dimension

  // LOGICAL VOLUME
  auto pore_LV = new G4LogicalVolume(pore_S,     // its solid
                                     mVacuum,    // its material
                                     "Pore_LV"); // its name
...
void DetectorConstruction::ConstructSDandField() {
  ...
  fEMfield_1 =
      new G4UniformElectricField(G4ThreeVector(0.0, 0.0, 1.0 * kilovolt / mm));
  ...
  auto fIntgrDriver_1 = new G4MagInt_Driver(fMinStep_1, fStepper_1,
                                            fStepper_1->GetNumberOfVariables());
  ...
  auto fIntgrDriver_2 = new G4MagInt_Driver(fMinStep_2, fStepper_2,
                                            fStepper_2->GetNumberOfVariables());
  ...
  fEMfield_WORLD =
      new G4UniformElectricField(G4ThreeVector(0.0, 0.0, 0.0 * kilovolt / m));
  ...
  auto fIntgrDriver_WORLD = new G4MagInt_Driver(
      fMinStep_WORLD, fStepper_WORLD, fStepper_WORLD->GetNumberOfVariables());
