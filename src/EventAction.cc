...
G4VHitsCollection *GetHitCollection(const G4Event *event, G4int hcID) {
  ...
  if (!hce) {
    ...
    G4Exception("EventAction::GetHitsCollection()", "Code001", JustWarning,
                msg);
    return nullptr;
  }
  ...
  if (!hc) {
    ...
    G4Exception("EventAction::GetHitsCollection()", "Code0001", JustWarning,
                msg);
  }
...
void EventAction::EndOfEventAction(const G4Event *event) {
  ...
  if (nHits > 0) {
    ...
    if (nLaminaHits > 0) {
      ...
      // auto primaryE =
      // event->GetPrimaryVertex()->GetPrimary()->GetKineticEnergy()/keV;
      ...
      fLaminaPosX = std::vector<G4double>(nLaminaHits, NAN);
      fLaminaPosY = std::vector<G4double>(nLaminaHits, NAN);
      fLaminaPosZ = std::vector<G4double>(nLaminaHits, NAN);
      for (unsigned long i = 0; i < nLaminaHits; i++) {
        ...
        fLaminaPosX[i] = hit->GetLocalPos().x() / mm;
        fLaminaPosY[i] = hit->GetLocalPos().y() / mm;
        fLaminaPosZ[i] = hit->GetLocalPos().z() / mm;
