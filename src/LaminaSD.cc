...
// Constructor
//****************************************************************************
LaminaSD::LaminaSD(G4String name) : G4VSensitiveDetector(name) {
...
void LaminaSD::Initialize(G4HCofThisEvent *hce) {
  ...
  fHitsCollection =
      new LaminaHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool LaminaSD::ProcessHits(G4Step *step, G4TouchableHistory *) {
  ...
  if (track->GetParticleDefinition() == G4Gamma::Definition() ||
    ...
    // if ( postStepPoint->GetProcessDefinedStep()->GetProcessType() == 2 ) { //
    // electromagnetic process
    auto edep = -1 * (postStepPoint->GetKineticEnergy() -
    ...
    if (edep > 0.) {
      ...
      // postTouch->GetHistory()->GetTopTransform(); transform.Invert();
